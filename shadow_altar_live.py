#!/usr/bin/env python3
"""
SHΔDØW‑FB‑FORGE · LIVE OTP RITUAL (PLAYWRIGHT EDITION)
Educational tool — use ONLY on authorized targets.
No external browser needed; Playwright brings its own Chromium.
"""
...
class BrowserForge:
    ...
    def start(self):
        ...
        if self.proxy:
            # Playwright expects proxy in format: { server: 'http://proxy:8080' }
            proxy_url = self.proxy
            if not proxy_url.startswith("http"):
                # assume socks5? Playwright only supports http/https proxies directly via 'server'
                # For socks5 we need to convert? Render might only have http proxies. We'll pass as is.
                proxy_url = self.proxy
...
# ---------- FACEBOOK CRAFTER (PLAYWRIGHT) ----------
class FacebookCrafter:
    SIGNUP_URL = "https://mbasic.facebook.com/reg"
    def __init__(self, browser: BrowserForge):
    ...
    def trigger_sms(self, first, last, password, phone):
        ...
        # select sex
        sex_option = random.choice(["1", "2"])
        page.select_option("select[name='sex']", sex_option) if page.query_selector("select[name='sex']") else None
        # birthday
